...
static const DUI_MSGMAP_ENTRY* DuiFindMessageEntry(const DUI_MSGMAP_ENTRY* lpEntry,TNotifyUI& msg )
{
	...
	CDuiString strMsgCtrlName = lpEntry->sCtrlName; // add by george
...
// 			if(!lpEntry->sCtrlName.IsEmpty())
		if (strcmp(lpEntry->sMsgType, sMsgType.GetData()) == 0)
		{
			if (!strMsgCtrlName.IsEmpty())
			{
...
void CWindowWnd::CenterWindow()
{
	...
	// 处理多显示器模式下屏幕居中
	MONITORINFO oMonitor; // = {}; mod by george
